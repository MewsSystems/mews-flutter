...
enum OptimusBasicCardVariant {
  /// The system default, general purpose option used in the majority of cases. It is recommended, but not required,
  /// to use it on a darker background for the best result.
  normal,

  /// The overlay type can only be used for dialogs, modals, or elements of a similar nature.
  /// It can contain nested cards.
  overlay,
}
...
enum OptimusCardSpacing { spacing0, spacing100, spacing200, spacing300, spacing400 }

enum OptimusCardAttachment { none, top, right, bottom, left }

/// A card is a container that groups related pieces of information. It is intended as a short,
/// linked representation of a conceptual unit.
///
/// The system default, general purpose option used in the majority of cases. It is recommended, but not required,
/// to use it on a darker background for the best result.
class OptimusCard extends StatelessWidget {
  ...
  /// Builds custom content. If content padding needed wrap in [OptimusCardChildPadding].
  final ContentWrapperBuilder contentWrapperBuilder;

  /// Attaches the card to one side. For example, a card attached to the right will set the top right and
  /// bottom right border-radius to 0.
...
/// linked representation of a conceptual unit.
///
/// The nested type, available in multiple variants, can only be used inside of a basic card type.
/// It is recommended to use only one layer of nesting.
class OptimusNestedCard extends StatelessWidget {
  ...
  Border get _border =>
      variant == OptimusNestedCardVariant.normal ? Border.all(width: 1, color: OptimusColors.basic500t16) : null;
  ...
  BorderRadius get _borderRadius {
    switch (attachment) {
      ...
      case OptimusCardAttachment.left:
        return const BorderRadius.only(topRight: borderRadius100, bottomRight: borderRadius100);
      case OptimusCardAttachment.right:
        return const BorderRadius.only(topLeft: borderRadius100, bottomLeft: borderRadius100);
      case OptimusCardAttachment.top:
        return const BorderRadius.only(bottomLeft: borderRadius100, bottomRight: borderRadius100);
      case OptimusCardAttachment.bottom:
        return const BorderRadius.only(topLeft: borderRadius100, topRight: borderRadius100);
    }
  }
}
...
class OptimusCardChildPadding extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: _padding, child: child);
